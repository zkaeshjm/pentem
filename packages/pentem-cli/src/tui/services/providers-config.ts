...
export function loadPersistedConfig(): PersistentConfig | null {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (m) (cfg as any)[m[1]] = m[2].replace(/^["']|["']$/g, '');
    }
...
export function detectFromEnvOrConfig(): { configured: boolean; provider: string; apiKey?: string; model?: string; baseUrl?: string; error?: string } {
  ...
  if (configured.length === 0) {
    return { configured: false, provider: '', error: 'No provider configured. Set ANTHROPIC_API_KEY or OPENAI_API_KEY.' };
  }
