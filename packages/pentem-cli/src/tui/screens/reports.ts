...
import { listReports, getReportContent } from '../services/workspace.ts';
...
export class ReportsScreen implements TUIScreen {
  ...
  constructor(parent: Widgets.BoxElement, app: App) {
    ...
    this.app.screen.key(['return'], () => { if (!this.reportBox) this.showReport(); });
    this.app.screen.key(['b', 'B'], () => { if (this.reportBox) this.showList(); });
  }

  activate(): void { this.box.show(); this.refresh(); this.list.focus(); this.app.screen.render(); }
  deactivate(): void { this.showList(); this.box.hide(); }

  refresh(): void {
    if (this.reportBox) return;
    const reports = listReports();
    this.list.setItems(reports.length === 0
      ? [' No reports found. Complete a scan to generate one.']
      : reports.map((r) => ` {bold}${r.sessionId.slice(0, 20).padEnd(20)}{/bold} | ${r.targetUrl.slice(0, 35).padEnd(35)} | ${(r.fileSize / 1024).toFixed(1)} KB`)
    );
  }
