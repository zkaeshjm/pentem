...
import type { AgentCategory, AgentResult, AgentType, ModelTier } from '@internal/shannon-shared';
import { AGENT_MAX_TURNS } from '@internal/shannon-shared';
...
const TIER_TO_MODEL: Record<ModelTier, string> = {
  small: process.env.SHANNON_MODEL_SMALL ?? 'haiku',
  medium: process.env.SHANNON_MODEL_MEDIUM ?? 'sonnet',
  large: process.env.SHANNON_MODEL_LARGE ?? 'opus',
};
