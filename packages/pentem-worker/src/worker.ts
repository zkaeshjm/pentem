...
async function main(): Promise<void> {
  ...
  const worker = await Worker.create({
    ...
    activities,
    taskQueue,
    identity: `pentem-worker-${taskQueue}`,
    maxConcurrentActivityTaskExecutions: 10,
    maxConcurrentWorkflowTaskExecutions: 5,
  });

  console.log(`[pentem-worker] Starting worker for task queue: ${taskQueue}`);
  console.log(`[pentem-worker] Temporal server: ${serverAddress}`);

  await worker.run();
}

main().catch((err) => {
  console.error('[pentem-worker] Fatal error:', err);
  process.exit(1);
});
