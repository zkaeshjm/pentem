...
function findConfigPath(): string | undefined {
  const candidates = [
    process.env.SHANNON_CONFIG,
    path.join(process.cwd(), 'shannon.yaml'),
    path.join(process.cwd(), 'shannon.yml'),
    path.join(process.cwd(), '.shannon.yaml'),
    path.join(process.cwd(), '.shannon.yml'),
    path.join(os.homedir(), '.shannon', 'config.yaml'),
    path.join(os.homedir(), '.shannon', 'config.toml'),
  ];
