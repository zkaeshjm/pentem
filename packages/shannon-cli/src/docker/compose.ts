...
export function generateCompose(options: ComposeOptions): string {
  ...
  const image = localDev ? 'shannon-worker:latest' : (imageTag ?? `shannon-worker:${taskQueue}`);
    ...
    services: {
      ...
      worker: {
        ...
        environment: {
          ...
          SHANNON_MODEL_SMALL: process.env.SHANNON_MODEL_SMALL ?? '',
          SHANNON_MODEL_MEDIUM: process.env.SHANNON_MODEL_MEDIUM ?? '',
          SHANNON_MODEL_LARGE: process.env.SHANNON_MODEL_LARGE ?? '',
        },
...
export function writeComposeFile(workspacePath: string, config: string): string {
  const dir = path.join(workspacePath, '.shannon');
